...
- 在其他 App 选中文本后唤起，可自动识别中英文并立即翻译；可编辑输入框会直接替换选区，不可编辑网页会复制结果。
- 输入英文 HTTPS 网页链接，可提取静态正文、分块翻译为中文，并可选生成中文摘要。
...
- 调用 MiniMax（Anthropic Messages 兼容接口），面板内先预览结果再粘贴或复制。
...
产物为已完整签名并验证的 `AIInput.app`。若钥匙串没有代码签名身份，脚本会使用仅限本机开发的稳定 ad-hoc designated requirement，让辅助功能授权可跨本地重新构建保持；正式分发请显式设置：

```bash
AIINPUT_SIGN_IDENTITY="Apple Development: Your Name (...)" ./build.sh
```

可用 `./build.sh --check-stable-dr` 连续构建两次并验证代码身份稳定。Codex App 的 Run 动作已连接到 `./script/build_and_run.sh`。
...
5. 在 Notes / Safari / VS Code / 任意输入框里按 `Ctrl+⌥+⌘+E`，选择模式与语气，输入内容后按 `Cmd+Return`。预览态再按一次会粘贴或复制，`⇧⌘C` 始终仅复制。

如果从旧的未正确签名版本升级，需在辅助功能列表中移除旧 `AIInput` 条目、运行新版并授权一次。此后使用同一 signing identity（或默认稳定开发 requirement）的重新构建不应重复索权。
...
| `Cmd+Return` | 翻译 / 润色 / 分析；预览态再按一次＝粘贴或复制 |
| `⇧⌘C` | 预览态：仅复制结果并关闭 |
| `Esc` | 关闭面板（处理中＝取消请求） |
| `↑` | 输入框为空时恢复上次原文 |
| 拖动面板空白处 | 移动面板 |
...
- 网页模式处理服务端已渲染、无需登录的 HTTPS 文章；纯 JavaScript 页面、登录墙和反爬页面可能无法提取正文。
- 本机开发用 ad-hoc requirement 只解决身份稳定，不提供正式分发的证书信任或防冒用能力。
